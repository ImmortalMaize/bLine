...
export default (props: {
  ...
  const { distance, pitch, points, stroke, index, time, slicelerator } = props
  ...
  const color =
    stroke
      .brighten(interpolate(frame, [
        convertedTime - 5,
        ...points.map((point: any) => slicelerator.slicelerate(time + (point.tick - points[0].tick), "framesPerTick")),
        slicelerator.slicelerate(time + (points[points.length - 1].tick - points[0].tick), "framesPerTick") + 10
      ], [
        ...
        .desaturate(interpolate(
          frame, [
              convertedTime + framesPerBar,
              convertedTime + framesPerBar*2
          ],
          [0, 3], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp"
    ...
    const interpolateOpacity = interpolate(frame, [convertedTime - framesPerBar*2, convertedTime, convertedTime + framesPerBar, convertedTime + framesPerBar * 2.5], [0, 1, 1, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.bezier(.48,.69,.5,1)
    })
    const interpolateOffset = interpolate(frame, [convertedTime - framesPerBar*1.5, convertedTime], [100, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.bezier(.5,0,.5,1)
    })
    const interpolateR = interpolate(frame, [convertedTime - framesPerBar*2, convertedTime - framesPerBar], [0, keyHeight / 2], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    ...
    const interpolateBlur = interpolate(
      frame, [
          convertedTime,
          convertedTime + framesPerBar*2.5
      ],
      [0, 3], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
  ...
  return <>
    {
      ...
      pitch.map((p: number, ind: number) => {
        ...
        if (points.length <= 2 && points[1].pitchBend === points[0].pitchBend) {
          ...
          const deadline: Time = slicelerator.slicelerate(time + (points[1].tick - points[0].tick), "framesPerTick")

          const tickRange = (points[1].tick - points[0].tick)
          const easing = tickRange > ticksPerBar / 4 ? Easing.linear : tickRange > ticksPerBar / 8 ? Easing.bezier(0,0.5,0.5,1): Easing.bezier(0,1,0,1)
          return <g name={"note-" + index + "-pitch-" + ind} opacity={interpolateOpacity}>
            ...
            <circle cx={
              interpolate(frame, [convertedTime, deadline], [distance, endpoint], {
                extrapolateLeft: "clamp",
                extrapolateRight: "clamp",
                easing
              })
        ...
        //if pitch bends, go CRAZEHH!!
          ...
          const timeTicks: Array<number> = points.map((point: any, index: number) => slicelerator.slicelerate(time + (point.tick - points[0].tick), "framesPerTick"))
